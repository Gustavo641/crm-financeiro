...
export default function DespesasPage() {
  ...
  const [despesas] = useState([
    { id: 1, descricao: 'Alimentação - Supermercado', valor: 450, data: '05/08/2025', categoria: 'Alimentação', status: 'Pago', banco: 'Nubank' },
    { id: 2, descricao: 'Conta de Internet', valor: 120, data: '10/08/2025', categoria: 'Internet', status: 'Pendente', banco: 'Itaú' },
    { id: 3, descricao: 'Uber para o trabalho', valor: 35.50, data: '12/08/2025', categoria: 'Transporte', status: 'Pago', banco: 'Nubank' },
  ]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Acompanhe todos os seus gastos</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-lg shadow p-6 border-l-4 border-red-500">
          <h2 className="font-semibold text-lg text-gray-900 mb-4">Registrar Nova Despesa</h2>
      ...
      <div className="flex items-center gap-3">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Buscar despesa..." className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500" />
        </div>
          ...
          <tbody className="divide-y divide-gray-200">
            {despesas.map((despesa) => (
              <tr key={despesa.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-6 py-3 text-sm font-semibold text-red-600">R$ {despesa.valor.toFixed(2)}</td>
                <td className="px-6 py-3 text-sm text-gray-600">{despesa.data}</td>
                <td className="px-6 py-3 text-sm text-gray-600">{despesa.categoria}</td>
                <td className="px-6 py-3 text-sm">
                  {despesa.status === 'Pago' ? (
                    <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium">🟢 {despesa.status}</span>
                  ) : (
                    <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-medium">🟡 {despesa.status}</span>
                  )}
                </td>
